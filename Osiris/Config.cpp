...
bool Config::loadScheduledFonts() noexcept
{
    ...
    for (const auto& fontName : scheduledFonts) {
        ...
        if (fonts.find(fontName) == fonts.cend()) {
            ...
            newFont.medium = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(fontData.get(), fontDataSize, 8.0f, &cfg, ranges);
            newFont.big = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(fontData.get(), fontDataSize, 8.0f, &cfg, ranges);
            fonts.emplace(fontName, newFont);
            result = true;
        }
#endif
