...
void Misc::PostPred(UserCmd* cmd)
{
    ...
    unpredicted_velocity = localPlayer->velocity().z;
}
...
void Misc::StaminaIndicator()
{
    ...
    static int lastvel, lastsavedvel;
    float currentstamina = localPlayer->stamina().length2D();
    static bool hm = false;
}
...
void Misc::EdgebugDetect(UserCmd* cmd) noexcept
{
...
void Misc::longjump(UserCmd* cmd) {
    if (miscConfig.longjump && miscConfig.edgejumpkey.isDown()) {

        if (!localPlayer || !interfaces->engine->isInGame() || !interfaces->engine->isInGame())
            return;
        ...
        if (localPlayer->flags() & 1) {
            saved_tick_count = memory->globalVars->tickCount;
        }

        if (memory->globalVars->tickCount - saved_tick_count > miscConfig.savedticks) {
            do_lj = false;
        }
        else {
            do_lj = true;
        ...
        if (do_lj && !(localPlayer->flags() & 1)) {
            cmd->buttons |= UserCmd::IN_DUCK;
        }
    }
}

void Misc::ladderlj(UserCmd* cmd)
{
    if (!miscConfig.ladderlj || !miscConfig.edgejumpkey.isDown())
        return;

    // acho que nao deveria funcionar assim
...
void Misc::smartedgebug(UserCmd* pCmd)
{
    ...
    float vel = gravity->getFloat() * 0.5 * memory->globalVars->intervalPerTick;

    if (!edgebugging) {

        for (int i = 0; i < miscConfig.edgebugtime; i++) {

            EnginePrediction::run(pCmd);
            {
                if (floor(localPlayer->velocity().z) == vel && unpredicted_velocity < -vel && localPlayer->moveType() != MoveType::LADDER) {
                    edgebugging = true;
...
void Misc::drawGUI2(bool contentOnly) noexcept
{
   ...
   // ImGui::Checkbox("Strafe Optimizer", &miscConfig.strafeopt);
    ...
    ImGui::PopID();
    if (miscConfig.edgebug)
        ImGui::SliderInt("Edgebug Time", &miscConfig.edgebugtime, 0, 64);
