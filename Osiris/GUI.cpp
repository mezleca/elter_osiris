    ...
    if (ImGui::ListBox("", &currentConfig, [](void* data, int idx, const char** out_text) {
        ...
        *out_text = (const char*)vector[idx].c_str();
        ...
        if (ImGui::Button("Create config", { 100.0f, 25.0f })) {
            memory->clientMode->getHudChat()->printf(0, std::string("").append(" \x0B").append("elterhook").append(" \x01").append("| Config Created").c_str());
        ...
        if (currentConfig != -1) {
            if (ImGui::Button("Load selected", { 100.0f, 25.0f })) {
                ...
                memory->clientMode->getHudChat()->printf(0, std::string("").append(" \x0B").append("elterhook").append(" \x01").append("| Config Loaded").c_str());
            }
            if (ImGui::Button("Save selected", { 100.0f, 25.0f })) {
                memory->clientMode->getHudChat()->printf(0, std::string("").append(" \x0B").append("elterhook").append(" \x01").append("| Config Saved").c_str());
                config->save(currentConfig);
            }
            if (ImGui::Button("Delete selected", { 100.0f, 25.0f })) {
                config->remove(currentConfig);
                memory->clientMode->getHudChat()->printf(0, std::string("").append(" \x0B").append("elterhook").append(" \x01").append("| Config Deleted :( :3 :3 :3").c_str());
...
void GUI::renderGuiStyle3() noexcept
{
    ...
    // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
    {
        ...
        ImGui::Begin("elterhook ", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
        {
            ...
            ImGui::BeginGroup();
            {
                if (gui2.tab("legit", tab == 0, ImVec2(150, 25)))
                    tab = 0;
                if (gui2.tab("triggerbot", tab == 1, ImVec2(150, 25)))
                    tab = 1;
                if (gui2.tab("glow", tab == 2, ImVec2(150, 25)))
                    tab = 2;
                if (gui2.tab("esp", tab == 3, ImVec2(150, 25)))
                    tab = 3;
                if (gui2.tab("visuals", tab == 4, ImVec2(150, 25)))
                    tab = 4;
                if (gui2.tab("miscellaneous", tab == 5, ImVec2(150, 25)))
                    tab = 5;
                if (gui2.tab("movement", tab == 6, ImVec2(150, 25)))
                    tab = 6;
                if (gui2.tab("inventory", tab == 7, ImVec2(150, 25)))
                    tab = 7;
                if (gui2.tab("configuration", tab == 8, ImVec2(150, 25)))
                    tab = 8;
                if (gui2.tab("backtrack", tab == 9, ImVec2(150, 25)))
                    tab = 9;
